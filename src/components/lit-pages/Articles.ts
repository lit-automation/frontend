...
export class Articles extends connect(window.store)(LitElement) {
        ...
        fetch(window.API_LINK + '/project/' + this.projectID + '/removeduplicates', {
            ...
            if (resp.status !== 200) {
                ...
                if (response.detail) {
                    ...
                    showNotification('Unable to remove duplicates');
                }
            } else {
                const response = await resp.json();
                showNotification('Removed ' + response.duplicates + ' duplicate articles from your article set.');
            }
        });
    }
    ...
    private downloadArticles = async (): Promise<string> => {
        const response = await fetch(window.API_LINK + '/project/' + this.projectID + '/article/download', {
            method: 'GET',
            mode: 'cors', // no-cors, *cors, same-origin
            cache: 'no-cache',
            headers: window.STDHeaders,

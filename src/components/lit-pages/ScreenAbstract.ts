...
export class ScreenAbstract extends connect(window.store)(LitElement) {
    ...
    private screen = (include: boolean): void => {
        ...
        fetch(window.API_LINK + '/project/' + this.projectID + '/screen/' + this.cudArtID+additional, {
            method: 'PUT',
            mode: 'cors', // no-cors, *cors, same-origin
            cache: 'no-cache',
            headers: window.STDHeaders,
    ...
    private autoScreen() {
        ...
        this.isLoading = true;
        fetch(window.API_LINK + '/project/'+this.projectID+'/screen/auto'+additional, {
            method: 'POST',
            mode: 'cors', // no-cors, *cors, same-origin
            cache: 'no-cache',
            headers: window.STDHeaders,
